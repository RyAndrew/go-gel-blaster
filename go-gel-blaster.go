...
var trexTiltMax int = 2400
var trexTiltMin int = 800
var trexTiltRange int = trexTiltMax - trexTiltMin
var trexTiltCurrentPos int

var trexPanMax int = 2400
var trexPanMin int = 800
...
func processClientMessage(client *WebSocketClient, message []byte) {
	...
	case "getPidConstants":
		...
		jsonData["p"] = fmt.Sprintf("%.2f", 1)
		jsonData["i"] = fmt.Sprintf("%.2f", 0)
		jsonData["d"] = fmt.Sprintf("%.2f", 0.5)
...
func main() {
	...
	fs := http.FileServer(http.Dir("/home/pi/go/src/github.com/RyAndrew/gelblaster/webroot"))
	http.Handle("/", fs)
